...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/04/27 10:22:42 by ntassin           #+#    #+#             */
/*   Updated: 2026/04/27 18:34:31 by ntassin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	alloc_map(t_map *map)
{
	...
	map->z = (int **)ft_calloc(map->height, sizeof(int *));
	map->color = (int **)ft_calloc(map->height, sizeof(int *));
	if (!map->z || !map->color)
		error_exit("Error: malloc failed");
	i = 0;
	while (i < map->height)
	{
		map->z[i] = (int *)ft_calloc(map->width, sizeof(int));
		map->color[i] = (int *)ft_calloc(map->width, sizeof(int));
